...
layout (location = 0) in vec3 aPos;	   // Definig position's buffer layout and defing the begin as slot 0
layout (location = 1) in vec3 aColor;  // Definig color's buffer layout and defing the begin as slot 1
layout (location = 2) in vec2 aTex;    // Definig texture's buffer layout and defing the begin as slot 2

out vec3 color;		// Defining a variable to store and export the color to the Fragment Shader
out vec2 texCoord;	// Defining a variable to store and export the textures coordinates to the Fragment Shader
