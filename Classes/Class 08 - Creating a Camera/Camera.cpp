...
void Camera::Matrix(float FOV, float nearPlane, float farPlane, Shader& shader, const char* uniform) // Initialize camera's matrix
{
	...
	projection = glm::perspective(glm::radians(FOV), (float)(width / height), nearPlane, farPlane); // Adds perspective to the scene
