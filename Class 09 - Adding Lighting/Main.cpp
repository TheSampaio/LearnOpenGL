...
#define LIGHT_POSITION              0.5f, 0.5f, 0.5f       // Light's position (0.5f, 0.5f, 0.5f)
...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window)) // Windows's loop
    {
        ...
        shaderProgram.Active();                    // Activate shader program (Pyramid)
        ...
        glDrawElements(GL_TRIANGLES, sizeof(indices) / sizeof(int), GL_UNSIGNED_INT, 0); // Tell OpenGL what to draw (Pyramid)
