...
int main()
{
    ...
    Shader* pShaderGamma = new Shader("Gamma.vert", "Gamma.frag");
    ...
    Texture* pTextureSpecularPlane = new Texture("../../Resources/Textures/specular-grass-01.jpg", GL_SRGB, GL_RED, GL_TEXTURE1);

    // Creates a material
    Material* pMaterialPlane = new Material(pTextureDiffusePlane, pTextureSpecularPlane, 0.04f, 8.0f);
    ...
    while (!window.Close())
    {
        ...
        pBufferFrame->Use(*pShaderGamma);
        Renderer::GetInstance().SetUniform1f(*pShaderGamma, "uGamma", gamma);
